...
<body>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0):
                        ...
                        while ($row = $result->fetch_assoc()): ?>
                            <tr data-id="<?= $row['id'] ?>" data-malop="<?= $row['maLop'] ?>"
                                data-mamonhoc="<?= $row['maMonHoc'] ?>" data-magv="<?= $row['maGV'] ?>">
    ...
    <script>
        ...
        // === Xử lý nhấn nút sửa (lấy dữ liệu từ <table> và điền vào form) ===
        document.addEventListener("click", (e) => {
            if (e.target.classList.contains("edit-btn")) {
                ...
                const maLop = tr.dataset.malop;
                const maMonHoc = tr.dataset.mamonhoc;
                const maGV = tr.dataset.magv;

                // Hiển thị popup thêm (tận dụng form có sẵn)
                showAddPopup();

                // Đổi tiêu đề và nút
                document.getElementById("title-h2").innerText = "CHỈNH SỬA PHÂN CÔNG";
                document.querySelector(".btn-primary").innerText = "Cập nhật";
                document.getElementById("formAction").value = "update";

                // Gán dữ liệu vào form
                document.querySelector("select[name='maLop']").value = maLop;
                document.querySelector("select[name='maMonHoc']").value = maMonHoc;
                document.querySelector("select[name='maGV']").value = maGV;

                // Thêm hidden input chứa ID phân công (nếu chưa có)
                let hidden = document.querySelector("input[name='id']");
                if (!hidden) {
                    hidden = document.createElement("input");
                    hidden.type = "hidden";
                    hidden.name = "id";
                    document.getElementById("addForm").appendChild(hidden);
                }
                hidden.value = id;
            }
        });
